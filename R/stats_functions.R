...
#' Compute test statistic that is based on the average rates of clonal exclusivity of a patient, and the observed
#' number of times a pair was clonally exclusive across several trees of the tree inference.
#'
#' For a given gene/pathway pair, this function takes as input: the average rates of clonal exclusivity of all patients
#' in which the pair is mutated, the number of trees among the trees in the collection of trees from each patient in which
...
#' @param avg_rates_m The vector of average rates of clonal exclusivity of each patient the pair is mutated in. It was computed for each patient
#' separately, and is averaged over all gene pairs and all trees. Expected to be in the same (patient) order as the other inputs to this function.
#' @param num_trees_pair The vector with the number of tree inferences in which the pair was occurring in. Has to be the same order
#' as \code{avg_rates_m}.
#' @param num_clon_excl The vector with the number of times the pair was clonally exclusive in the trees in each patient.
...
#' maxLik
#' stats
#' @return A list with the test statistic of the clonal exclusivity test (lrtest), and the maximum likelihood estimate
#' of delta.
#' @examples
#' compute_test_stat_avg_rate(c(0.1, 0.2), c(10, 10), c(9, 7))
#' compute_test_stat_avg_rate(c(0.05, 0.23), c(20, 20), c(8, 5))
compute_test_stat_avg_rate <- function(avg_rates_m, num_trees_pair, num_clon_excl){
  ...
  # this should anyways not be the case. If the rate is 0, there should be no pair that is clonally exclusive
  # at all
  for(i in seq(1, num_shared_pats)){
  if(avg_rates_m[i] == 0 && num_clon_excl[i] == num_trees_pair[i]){
    stop(paste("[Function: compute_test_stat_avg_rate]: The rate is 0, but there is a pair which is clonally ",
               "exclusive across ALL trees, i.e. num_clon_excl == num_trees_pair, which is == ", num_trees_pair[i],
               ". This is not possible. Make sure that the rates are in the same patient order as the ",
               "histogram of pairs across trees, or that the rates are not distorted too much by the beta distribution.",
               sep = ""))
    }
  }

  # compute the test statistic of the log-likelihood ratio test:
  # H0 = clonal exclusivity just by chance, explained by the general clonal exclusivity rates in
  #  the shared patients, likelihood: s_1_{A,B} * s_2_{A,B} * s_3_{A,B} if the current pair {A,B} occurred in patients 1, 2, 3
  # each of those s_i_{A,B} = (num_clon_excl/num_trees)*m_i + ((num_trees - num_clon_excl)/num_trees)*(1-m_i)
  #
  # H1 = clonal exclusivity at elevated rates, logit(m_star_i) = logit(m_i) + delta, likelihood: s_star_1_{A,B} * s_star_2_{A,B} * s_star_3_{A,B}
  #  take mle estimate for the delta
  # LRTestStatistic ~ Chi^2 with 1 degree of freedom (under H0) OR compare it to the empirical distribution under the null
  ...
  compute_weighted_p <- function(num_cl, num_trees, avg_m){
    ...
    res <- (num_cl/num_trees)*avg_m + ((num_trees-num_cl)/num_trees) * (1-avg_m)
    stopifnot(is.numeric(res))
    stopifnot(res >= 0 && res <= 1)
    return(res)
  }
  # compute individual factors from each patient
  l_null_factors <- apply(cbind(num_clon_excl, num_trees_pair, avg_rates_m), 1, function(x){compute_weighted_p(x[1], x[2], x[3])})
  ...
  logit_avg_rates_m <- gtools::logit(avg_rates_m) # without the gtools function, this crashes when one of the rates is 1.0

  # They should be identical: i.e. if delta is zero, m and m_star should be the same
  if(abs( (gtools::inv.logit(logit_avg_rates_m))[1] - avg_rates_m[1]) >= 0.01){
    stop("The sanity check with delta = 0 did not have the right result. m should be identical to m_star if delta = 0!!")
  }

  # define the log-likelihood function
  this_LL <- function(this_delta) {
    avg_rates_m_star <- gtools::inv.logit(logit_avg_rates_m + this_delta)
    l_alt_factors <- apply(cbind(num_clon_excl, num_trees_pair, avg_rates_m_star), 1, function(x){compute_weighted_p(x[1], x[2], x[3])})
  ...
  message(paste("Maximum likelihood estimate of delta given the data is: ", mle_delta, sep = ""))
...
#'
#' @title Compare observed likelihood ratio test statistic to its ecdf under null.
#' @param entA One gene/pathway of the pair.
#' @param entB The other gene/pathway of the pair.
#' @param clone_tbl The clone tibble as generated with \code{\link{create_tbl_tree_collection}} from several trees of
#' the tree inference, i.e. it also contains a column 'tree_id'.
#' @param avg_rates_m The average rates of clonal exclusivity for each patient. The name of each rate is the respective patient_id.
#' @param ecdf_list The list of ECDF's of the test statistic under the null distribution. Can be generated with \code{\link{generate_ecdf_test_stat}}.
...
#' @author Ariane L. Moore
#' @return Returns list(p_val, num_patients, mle_delta, test_statistic), i.e. a list
...
#'                                   ecdf_list, "greater")
ecdf_lr_test_clon_excl_avg_rate <- function(entA, entB, clone_tbl, avg_rates_m, 
  ...
  message(paste("There were ", num_pats, " different patient ids.", sep = ""))

  # get the number of trees
  all_tree_ids <- unique(as.character(clone_tbl$tree_id))
  num_trees <- length(all_tree_ids)
  message(paste("There were ", num_trees, " different tree inferences.", sep = ""))
  
  # check in which patients the current pair is mutated, and in wich trees
  # this is the list of trees, where each list entry is the vector of patient ids
  ...
  tree_to_pats_ent_pair <- lapply(all_tree_ids, function(this_tree){
    ...
    these_pats_mutated <- lapply(c(entA, entB), function(this_ent){
      ...
      this_ent_pats <- unique(as.vector(as.data.frame(this_ent_pats_tbl)[,1]))
      stopifnot(dim(this_ent_pats_tbl)[1] == length(this_ent_pats))
      this_ent_pats
    })
    # now only those where both of them are mutated
    these_pats_both_ents_mutated <- intersect(these_pats_mutated[[1]], these_pats_mutated[[2]])
  ...
  # # check in which patients the current pair is mutated, and in wich trees
  # cnt = 0
  # tree_to_pats_ent_pair <- list() # this is the list of trees, where each list entry is the vector of patient ids
  # # in which both of them are mutated
  ...
  # the number of patients in which they are both mutated (maximum of all trees)
  ...
  if(num_shared_pats_total == 0){
    ...
    these_shared_pats_rates <- sapply(shared_pats_total, function(x) {
      avg_rates_m[which(names(avg_rates_m) == x)]
    })


    # compute for all patients: number of trees the pair is mutated, and number of times it was also clonally exclusive
    num_trees_pair_num_clon_excl <- vapply(shared_pats_total, function(x){
      this_clone_tbl <- clone_tbl %>%                     # take the clone tibble
        dplyr::filter(patient_id == x) %>%                      # just for the current patient
    ...
    # compute the test statistic and the p-value of the likelihood ratio test
    # by comparing the observed test statistic to the ecdf of the test statistic under the null
    res_lr_test <- c()
    res_lr_test <- compute_test_stat_avg_rate(these_shared_pats_rates, num_trees_pair, num_clon_excl)
    ...
    if(alternative == "greater" && mle_delta <= 0){
      message(paste("The current gene/pathway pair: ", entA, ", " , entB,
                  ...
                  "Since the alternative is 'greater', no test will be performed.", sep = ""))
      return(list(pval = ".", num_patients = num_shared_pats_total, mle_delta = mle_delta, test_statistic = test_stat))
    } else {
      ...
      if(num_shared_pats_total > length(ecdf_list)){
        stop(paste("[Function: ecdf_lr_test_clon_excl_avg_rate] The length of the ecdf list is not long enough. The current ",
                 "pair ", entA, " and ", entB, " seems to be mutated together in more than ",length(ecdf_list),
                 " patients. More precisely, they are mutated together in ", num_shared_pats_total,". ",
                 "Please re-run the step for generating the ecdf and use a higher value for num_pat_pair_max ",
                 "(at least ", num_shared_pats_total,").", sep = ""))
      }
      message(paste("Test statistic is ", test_stat, sep = ""))
      message(paste("The number of patients in which both genes are mutated: " , num_shared_pats_total, sep = ""))
      ...
      message(paste("The value of the ecdf at the test statistic: ", this_ecdf(test_stat), sep = ""))

      p_val <- 1-this_ecdf(test_stat)
      message(paste("p_val = ", p_val, sep=""))

      return(list(p_val = p_val, num_patients = num_shared_pats_total, mle_delta = mle_delta, test_statistic = test_stat))
    }
  }
}
