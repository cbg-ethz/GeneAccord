...
#'
#' In addition to the average rate of clonal exclusivity, it also visualizes the average number of
#' clones of each patient.
#'
#' @title Barplot of rates of clonal exclusivity and number of clones.
#' @param avg_rates_m A named vector with the average rates of clonal exclusivity for each patient. The name of each
#' element is the patient id to be used in the barplot.
#' @param clone_tbl The tibble containing the gene-to-clone assignments from all patients and all trees from the collection
#' of trees.
...
#'
#' The ECDF's of the test statistic under the null for a data set can be generated with \code{\link{generate_ecdf_test_stat}}. 
#' Afterwards, they can be visualized with this function. It is assumed that the first ECDF in the ecdf_list is
#' the ECDF for the case where pairs are mutated in two patients.
#'
#' @title Plot empirical cumulative distribution functions of the test statistic under the null.
#' @param ecdf_list The list of ECDF's as generated with \code{\link{generate_ecdf_test_stat}}.
#' @param plot_idx The index of which of the list entries of the ecdf_list to plot. Default: c(2,3).
#' @param num_panel_rows The ECDF's will be plotted altogether, hence \code{par(mfrow=c(x, y))} is used.
#' Here, \code{x} is the number of panel rows, which has to be set with this parameter, and \code{y} will be taken as
#' ceil(#ECDF's/x). E.g., if you have 20 ECDF's in total, you can set \code{num_panel_rows = 4}, and then your 20
#' ECDF's will be plotted in panels with four rows, and five columns. Default = 1.
#' @param output_pdf The name of the pdf to be generated. Or if output_pdf is "direct", then the plot is
#' generated directly and not to a pdf. Default: "direct".
#' @author Ariane L. Moore
#' @import 
...
#'
#' @title Plot histogram and empirical cumulative distribution function of p-values.
#' @param res_sim tibble containing the simulated pairs of genes/pathways. It contains the columns 'num_patients',
...
vis_pval_distr_num_pat <- function(res_sim, output_pdf = "direct"){
  ...
  par(mfrow = c(1,2), oma = c(3, 3, 3, 3)) #  make the outer margin at the bottom of the plot large

  # histogram
  hist(p_values, main = "Histogram of p-values", xlab="P-values", breaks = 25, xlim = c(0,1))

  ##
  # qq-plot
  # check how many observations there are for the different num_patients
  num_patients_tally <- res_sim %>% dplyr::select(num_patients) %>% dplyr::group_by(num_patients) %>% dplyr::tally()
  ...
  plot(cbind(sort(runif(length(p_vals_idx))), sort(this_num_pat_pvals[p_vals_idx])), main="Uniform Q-Q Plot", xlab="Theoretical Quantiles",
         ylab="Sample Quantiles", xlim = c(0,1), ylim = c(0,1), col = num_patients_colors[1], pch = ".")
  ...
  if(number_different_patients > 1){
    for(i in 2:number_different_patients){
      ...
      points(sort(runif(length(p_vals_idx))), sort(this_num_pat_pvals[p_vals_idx]), col = num_patients_colors[i], pch = ".")
  ...
  # overlay the entire figure region with a new, single plot. Then call legend with the location ("bottom", ...)
  par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0), new = TRUE)
  plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
  legend("bottom", inset = c(0, 0), xpd = TRUE, horiz = TRUE,
                legend = these_num_patients, col = num_patients_colors, pch = 19, pt.cex = 0.8, title = "Number of patients")
...
#' @param pairs_of_interest The tibble containing the pairs of genes/pathways that should be visualized in the heatmap.
...
#' @param first_clone_is_N Logical indicating whether the first clone column is actually representing the normal or germline, and is not a tumor
...
heatmap_clones_gene_pat <- function(pairs_of_interest, clone_tbl, all_genes_tbl, first_clone_is_N = FALSE, output_pdf = "direct"){
  ...
  if("tree_id" %in% colnames(clone_tbl)){
    stop(paste("Can only plot for one tree at a time. Make sure that the clone tbl is just from one tree id, and does not contain the column 'tree_id'", sep = ""))
  }
  ...
  # if the column labeled 'clone1' is actually the normal/germline, the cancer clone numbers are also reduced by one
  clone_col_names <- colnames(clone_tbl)[grepl("clone", colnames(clone_tbl))]
  ...
  for (this_col in clone_col_names){
    if(first_clone_is_N){
      if (this_col == "clone1"){
        ...
        this_new_col <- paste0("Clone", as.numeric(sub("clone", "", this_col))-1)
      }
    } else {
  ...
  colnames(clone_tbl)[grepl("clone", colnames(clone_tbl))] <- new_clone_col_names

  # here we remove columns that are just clone zero in all patients, for plotting
  filterd_clone_tbl_just_clones <- clone_tbl %>% dplyr::select(-file_name, -patient_id, -altered_entity)
  ...
  filterd_clone_tbl_goi <- filterd_clone_tbl_nonzero_clones %>% dplyr::filter(altered_entity %in% genes_of_interest)


  # map the ensembl gene ID's to the gene names
  these_ens_ids <- as.vector(as.data.frame(filterd_clone_tbl_goi %>% dplyr::select(altered_entity))[,1])
  hgnc_symbols <- c()
  ...
  filterd_clone_tbl_goi_gene_names <- cbind(as.data.frame(filterd_clone_tbl_goi), hgnc_symbols)
  ...
  all_pats <- unique(as.character(as.vector(as.data.frame(filterd_clone_tbl_goi_gene_names %>%
  ...
  for (this_pat in all_pats){
    ...
    tbl_to_plot_just_clones <- tbl_to_plot %>% dplyr::select(-hgnc_symbols)
    ...
    tbl_to_plot_melted <- suppressMessages(reshape2::melt(tbl_to_plot_nonzero_clones))

    colors <- rev(c("darkblue", "lightblue"))
    this_plot <- ggplot2::ggplot() +
      ggplot2::geom_tile(data=tbl_to_plot_melted, ggplot2::aes(x = hgnc_symbols, y = variable, fill = factor(value))) +
      ggplot2::geom_tile(data=tbl_to_plot_melted, ggplot2::aes(x = hgnc_symbols, y = variable), size=1, fill=NA, color="black") +
      ggplot2::scale_fill_manual(values = colors, name = "Mutation status") +
      ggplot2::theme_minimal() +
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.5, hjust = 1)) + # Vertical text on x axis
  ...
  if (cnt > 156) {
    ...
    my_letters <- c(LETTERS,
      ...
      my_letters <- LETTERS[1:cnt]
  }
  plot_altogether <- ggpubr::ggarrange(plotlist = plot_list, ncol = cnt, labels = my_letters)
