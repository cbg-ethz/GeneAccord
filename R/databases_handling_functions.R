...
#'
#' This function retrieves the ensembl gene id's from biomart together with the hgnc gene symbol,
#' the entrez gene id, the uniprot/swissprot gene id, as well as chromosome, start and end position.
#' This is done for the human genes from the human genome version \code{hg19}/\code{GRCh37}, and
#' Ensembl Genes version 88. The user can also specify other human genome or ensembl versions.
#'
#' @title Get a tibble of all gene ensembl id's, gene names (hgnc), entrez gene id's, uniprot/swissprot
#' gene id's and genomic coordinates.
#' @author Ariane L. Moore
#' @return A tibble with the following columns: \code{ensembl_gene_id}, \code{hgnc_symbol},
#' \code{entrezgene}, \code{uniprotswissprot}, \code{chromosome_name}, \code{start_position},
#' \code{end_position}. The entrez gene id, as well as the start and end positions are numeric, and
#' the other columns are characters. The chromosome is specified without "chr", i.e. the chromosome
  ...
  all_genes_tbl <- tryCatch(
    {
      ...
      message("Retrieve ensembl gene id's and hgnc symbols from biomart...")
      all_genes <- biomaRt::getBM(attributes=c('ensembl_gene_id', 'hgnc_symbol', 'entrezgene', 'uniprotswissprot',
    ...
    error=function(cond) {
      message("BiomaRt function call did not work. Possibly biomart or ensembl are currently down.")
    ...
    warning=function(cond) {
      message("BiomaRt function call caused a warning. Possibly biomart or ensembl are currently down.")
...
#'
#' For an ensembl id and a tibble with all genes as input, this function returns the matching
#' hgnc gene symbol. The tibble with all genes can be generated with
...
#' @param all_genes_tbl A tibble with all genes ensembl id's and hgnc symbols.
#' @author Ariane L. Moore
...
ensembl_to_hgnc <- function(this_ensembl, all_genes_tbl){
  ...
  this_hgnc_tbl <- all_genes_tbl %>% dplyr::filter(ensembl_gene_id == this_ensembl) %>%
      dplyr::select(hgnc_symbol)
  this_hgnc_character <- unique(as.vector(as.data.frame(this_hgnc_tbl)[,1]))
  num_gene_symbols = length(this_hgnc_character)

  if (num_gene_symbols == 0){  # i.e., if there was no such ensembl id
    message(paste("Ensembl id ", this_ensembl," cannot be found in the tibble!", sep = ""))
    return(this_ensembl)
  } else {
    if (num_gene_symbols > 1){
      warning(paste("Ensembl id ", this_ensembl,
                    " was mapped to several HGNC symbols:", sep = ""))
      for (i in 1:num_gene_symbols){
      ...
      if (this_hgnc_character == ""){
        message(paste("The HGNC symbol for this Ensembl id ", this_ensembl," is empty!"))
        return(this_ensembl)
      } else{
...
#'
#' For a hgnc gene symbol and a tibble with all genes as input, this function returns the matching
#' ensembl gene id. The tibble with all genes can be generated with \code{\link{create_ensembl_gene_tbl_hg}}.
#'
#' @title Get the ensembl gene id for a hgnc gene symbol.
#' @param this_hgnc The hgnc gene symbol of a gene.
#' @param all_genes_tbl A tibble with all genes ensembl id's and hgnc gene symbols.
#' @author Ariane L. Moore
#' @return The matching ensembl gene id. In case several ensembl gene id's were found, they
...
hgnc_to_ensembl <- function(this_hgnc, all_genes_tbl){
  ...
  stopifnot("ensembl_gene_id" %in% colnames(all_genes_tbl))
  stopifnot("hgnc_symbol" %in% colnames(all_genes_tbl))
  this_ensembl_tbl <- all_genes_tbl %>% dplyr::filter(hgnc_symbol == this_hgnc) %>%
    dplyr::select(ensembl_gene_id)

  this_ensembl_character <- unique(as.vector(as.data.frame(this_ensembl_tbl)[,1]))
  this_ensembl_character <- this_ensembl_character[which(this_ensembl_character != "")]
  num_ensembl_ids = length(this_ensembl_character)

  if (num_ensembl_ids == 0){  # i.e., if there was no such ensembl id or the entrez id
    # was empty
    message(paste("hgnc symbol ", this_hgnc,
                  " cannot be found in the tibble or there was no matching ensembl id!"
                  , sep = ""))
    return(this_hgnc)
  } else {
    if (num_ensembl_ids > 1){
      warning(paste("hgnc symbol ", this_hgnc,
                    " was mapped to several ensembl id's.", sep = ""))
      toReturn <- this_ensembl_character[1]
      for (i in 2:num_ensembl_ids){
        toReturn <- paste(toReturn, ";", this_ensembl_character[i], sep = "")
      }
      return(toReturn)
...
#' For a tibble that contains the information which ensembl gene id is mutated in which clone, map the ensembl
#' gene id to the reactome pathways that contain this gene.
#'
#' Such a tibble can be generated with e.g. the function \code{\link{create_tbl_tree_collection}}. If the altered entities
#' in the lists were the ensembl gene id's, this function can convert the tibble into a tibble with the altered
#' reactome pathways. It has the columns 'file_name', 'patient_id', 'altered_entity', 'clone1', 'clone2', ... up to the maximal
#' number of clones (Default: until 'clone7'). If the mutated entities are ensembl gene id's, they can be mapped
#  with this function to the pathways from 'reactome'. The pathways are from the lowest level of hierarchy.
#'
#' @title Map ensembl gene id clone tibble to reactome pathway clone tibble.
#' @param mutated_gene_tbl The tibble containing the information of which ensembl gene id is altered in which patient
#'  and clone. Can be created with e.g. \code{\link{create_tbl_ent_clones}}.
#' @param ensg_reactome_path_map A tibble with all ensembl id's and their reactome pathways. Can be loaded with \code{data("ensg_reactome_path_map")}.
#' @author Ariane L. Moore
#' @return The tibble containing the information of which pathway is altered in which clone.
#' @import
#' dplyr
#' @examples
...
#' patient_id = c("1","1"),
#' altered_entity = c("ENSG00000134086", "ENSG00000141510"),
#' clone1 = c(1,0),
#' clone2 = c(0,1))
#' convert_ensembl_to_reactome_pw_tbl(mutated_gene_tbl, ensg_reactome_path_map)
convert_ensembl_to_reactome_pw_tbl <- function(mutated_gene_tbl, ensg_reactome_path_map){
  altered_entity <- NULL
  ...
  if("tree_id" %in% colnames(mutated_gene_tbl)){
    stop("Please do the gene-to-pathway mapping separately for each tree inference.")
  }
  ...
  # message to user
  message(paste("Found ", num_all_unique_ents, " different Ensembl gene id's in the provided alteration tibble.",
                sep = ""))
  ...
  ensembl_to_pw_list <- sapply(all_unique_ents, function(x){
  ...
  converted_tbl_list <- apply(mutated_gene_tbl, 1,
                              function(y) {
                                ...
                                for(i in seq(1, num_clones)){
                                  ...
                                  names(this_converted_tbl)[names(this_converted_tbl) == "new_col"] <- clone_name
                                }
  ...
  # i.e. we have in the same patient the same mutated entity (here pathway)
  # possibly with different clone assignments
  ...
  message(paste("The remaining ", num_all_unique_ents-num_unmapped_ids, " different Ensembl gene id's were",
                ...
                "\nThere are ", num_pws, " clone-pathway associations in the converted tibble.", sep = ""))
...
#'
#' As input, an ensembl gene id is given as well as the tibble 'ensg_reactome_path_map'. It can be loaded with
#' \code{data("ensg_reactome_path_map")}, and contains the ensembl gene id to reactome pathway mappings. The reactome
#' pathways are from the lowest level of the hierarchy. This function returns the reactome pathways for the input gene.
#'
#' @title Get the reactome pathways for an ensembl gene id.
#' @param this_ensembl The ensembl id of a gene.
#' @param ensg_reactome_path_map A tibble with all ensembl id's and their reactome pathways. Can be loaded with \code{data("ensg_reactome_path_map")}.
#' @author Ariane L. Moore
#' @return The pathways that contain this gene as a character vector.
#' @import 
#' dplyr
#' @examples
#' data("ensg_reactome_path_map")
...
ensembl_to_reactome <- function(this_ensembl, ensg_reactome_path_map) {
  ...
  these_pws_tbl <- ensg_reactome_path_map %>% dplyr::filter(ensembl_gene_id == this_ensembl) %>%
      dplyr::select(reactome_pw_name)
  # convert to character vector
  these_pws<-unique(as.character(as.vector(these_pws_tbl$reactome_pw_name)))

  # check how many pathways were found
  num_pws = length(these_pws)

  if (num_pws == 0){  # i.e., if there was no such ensembl id in the tibble
    ...
    message(paste("The ensembl ID ", this_ensembl,
                  " was mapped to ", num_pws, " reactome pathways.", sep = ""))
    return(these_pws)
  }
}
