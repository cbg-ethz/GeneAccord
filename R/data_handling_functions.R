...
#' @param path_to_file The path to the file with the table of altered genes/pathways and their clone affiliation.
#' @param max_num_clones The upper bound for the number of clones that were found per tumor. Default: 7.
#' @author Ariane L. Moore
#' @return The tibble containing the information of which gene/pathway is altered in which clone in a patient.
...
create_tbl_ent_clones <- function(path_to_file, max_num_clones = 7){
  ...
  message(paste("Found the file ", path_to_file, sep = ""))

  # create tibble
  fn <- path_to_file
  file_name_tibble <- paste(tail(unlist(strsplit(fn, "/")), n = 2), collapse="/")
  this_csv <- read.csv(fn)
  all_ents <- as.character(as.vector(this_csv[,1]))
  this_clone_tbl <- tibble::tibble(file_name = file_name_tibble,
                                     patient_id = sub(".csv", "", basename(fn)),
  ...
  for(i in 1:num_clones){
    ...
    this_clone_tbl <- this_clone_tbl %>%
                         tibble::add_column(new_col = as.integer(this_csv[,(i+1)]))
                          names(this_clone_tbl)[names(this_clone_tbl) == "new_col"] <- clone_name
  }
  stopifnot(num_clones <= max_num_clones)
  # to make sure that all tibbles from all patients contain the same number of
  # clones, pseudo-columns with only zeros are added if the number of clones
  ...
  stopifnot(num_clones == max_num_clones)
  message(paste("Found ", num_entries, " altered genes/pathways in total.", sep = ""))
...
#'
#' Creates a tibble containing the information of which genes/pathways are altered in which clone in a patient across a collection
#' of tree inferences. It expects a list containing the paths to the comma-separated tables where the first column is the name of the
#' altered gene or pathway. The other columns are for the clones in the respective tumor. Such tables can be generated
#' by repeatedly performing the phylogenetic tree inference with e.g. the package \code{Cloe}, or by sampling from the
#' posterior. The tables are expected to be comma-separated and to have the columns 'altered_entity', 'clone1', 'clone2',
#' ..., 'cloneN', depending on how many clones were detected in the respective tumor. Each row then contains
#' in the first column the name of the mutated gene or affected pathway, e.g. "ENSG00000134086", and in the other 
#' columns it has either zeros or ones, indicating in which clone the respective gene/pathway is altered.
#' 
#' @title Get clone alteration tibble across the collection of trees.
#' @param input_files A vector containing the paths to the files with the tables of altered genes/pathways and their clone affiliation from
#' the collection of tree inferences. 
#' @param no_noisy_ents Minimum fraction for genes/pathways of how often they have to occur across the collection of trees in order to be in the
#' tibble. This makes sure that noisy genes, which were not assigned to many trees are excluded. Default: 0.9.
#' @param max_num_clones The upper bound for the number of clones that were found per tumor. Default: 7.
#' @author Ariane L. Moore
#' @return A clean tibble with the information of which gene/pathway is altered in which clone in the patient, and with
#' an entry for each tree inference where it occurred. Has the columns 'file_name', 'patient_id', 'altered_entity', 'clone1', 
#' 'clone2', ... up to the maximal number of clones (Default: until 'clone7'), and 'tree_id' as an indication in which tree the 
#' assignment was found. Note that the labelling of the clones does not matter and only needs to stay fixed within each patient and 
#' tree inference. 
#' @import 
#' dplyr
#' @examples
...
create_tbl_tree_collection <- function(input_files, no_noisy_ents = 0.9, max_num_clones = 7){
  ...
  stopifnot(is.numeric(max_num_clones))
  sanity_check <- sapply(input_files, function(x){stopifnot(is.character(x)); stopifnot(file.exists(x))})
  
  num_files <- length(input_files)
  message(paste0("Found a collection of tree inferences of ", num_files, " input files."))
  
  # go over the files, create a tibble with the additional information from which tree collection the tbl is
  all_tbl_list <- list()
  for(i in seq(1, num_files)){
   this_tbl <- suppressMessages(create_tbl_ent_clones(input_files[i], max_num_clones = max_num_clones))
   all_tbl_list[[i]] <- this_tbl %>% dplyr::mutate(tree_id = i)
  }
  # this is now the clone tibble from just that patient but from all tree inferences
  all_trees_clone_tbl <- do.call("rbind", all_tbl_list)
  
  # now exclude genes that occur in less than 90% of the trees, e.g. less than 18 of 20
  all_ents <- unique(all_trees_clone_tbl$altered_entity)
  histogram_ents <- all_trees_clone_tbl %>% dplyr::select(altered_entity) %>%
    dplyr::group_by(altered_entity) %>% dplyr::tally()
  noisy_ent_thresh <- no_noisy_ents*num_files
  # get a vector with all genes that are stably assigned to the trees
  stable_ents <- as.data.frame(histogram_ents %>% dplyr::filter(n >= noisy_ent_thresh) %>% 
                                 dplyr::select(altered_entity))$altered_entity
  # filter such that only those are included
  all_trees_clone_tbl <- all_trees_clone_tbl %>% dplyr::filter(altered_entity %in% stable_ents)
  num_genes_filtered_out <- length(all_ents)-length(stable_ents)
  if (num_genes_filtered_out == 1){
    message(paste("Removed ", num_genes_filtered_out, " gene that is missing in more than ", 
                  (num_files-noisy_ent_thresh) ," of gene-to-clone assignments.", sep = ""))
  } else {
    message(paste("Removed ", num_genes_filtered_out, " genes that are missing in more than ", 
                (num_files-noisy_ent_thresh) ," of gene-to-clone assignments.", sep = ""))
  }
  ...
  stopifnot(num_clones == max_num_clones)
  message(paste("Found ", num_entries, " altered genes/pathways in total, which are from ", num_files," gene-to-clone assignments.", sep = ""))
...
#'
#' Takes the gene-to-clone assignment tibble as created with \code{\link{create_tbl_tree_collection}} and computes for each instance from
#' the collection of trees the rate of clonal exclusivity. This rate is the fraction of gene/pathway pairs that were on a different branch
#' in the tumor phylogeny, i.e. the fraction of pairs that was clonally exclusive.
#' 
#' @title Get rates of clonal exclusivity for each tree inference
#' @param pat_tbl A tibble with the information of which gene/pathway is altered in which clone in the patient, and including this information
#' from the collection of trees. Can be created with with \code{\link{create_tbl_tree_collection}}. 
#' @author Ariane L. Moore
#' @return A vector with all rates of clonal exclusivity from all tree inferences. 
#' @import 
#' dplyr
#' @examples
...
compute_rates_clon_excl <- function(pat_tbl){
  ...
  # extract how many different instances are in the collection of trees
  all_trees <- unique(pat_tbl$tree_id)
  num_trees <- length(all_trees)
  message(paste0("Found ", num_trees, " different tree instances in the gene-to-clone tibble from patient ", pat_id, "."))
  message("Will compute the rate of clonal exclusivity for each of these...")
  
  # compute the rate of clonal exclusivity for each tree inference
  all_rates_across_tree_collection <- c()
  for (this_tree in all_trees){
    this_tree_pat_tbl_wo_pat_name <- pat_tbl %>%
      dplyr::filter(tree_id == this_tree) %>%
      dplyr::select(-file_name, -patient_id, -tree_id)
    this_tree_rate_this_pat <- suppressMessages(get_rate_diff_branch_ent_pair(this_tree_pat_tbl_wo_pat_name))
    all_rates_across_tree_collection <- c(all_rates_across_tree_collection, this_tree_rate_this_pat)
  }
  message("...done")
  ...
  message(paste("Current patient ", pat_id, " has mean rate ", round(mean_rate, digits = 3), 
                ", median rate " , round(median_rate, digits = 3), ", and the rate is zero in ", rate_is_zero, " out of ", num_trees, " trees.", sep = ""))
...
#'
#' Given the output of a tool that identifies clones within tumors and their phylogenetic
#' history, this function computes the rate of mutated gene/pathway pairs being in different branches.
#' That is, it will calculate the number of times mutated gene/pathway pairs are in different branches/clones
#' divided by the total number of all mutated gene/pathway pairs.
#'
...
#' @author Ariane L. Moore
#' in the tumor, e.g. 'clone1', 'clone2', 'clone3'. This tibble can be generated e.g. from the \code{Cloe} output.
#' @return The rate of occurrence of mutated gene/pathway pairs being in different clones.
#' @import 
#' caTools
#' dplyr
#' @examples
...
get_rate_diff_branch_ent_pair <- function(clone_tbl) {
  ...
  if("patient_id" %in% colnames(clone_tbl) || "file_name" %in% colnames(clone_tbl) || "tree_id" %in% colnames(clone_tbl)){
    stop("The clone tibble is expected to have only the altered_entity and clones columns, but not file_name, patient_id, or tree_id!")
  }
  ...
  if(num_ents == 1){
    message(paste("Found only 1 mutated gene/pathway in the provided tibble.",
                  " The rate of being on a different branch cannot be determined, but will be set to 0", sep = ""))
    return(0)
  } else {
    message(paste("Found ", num_ents, " different mutated genes/pathways in the provided tibble.", sep = ""))
    
    num_clones <- dim(clone_tbl)[2]-1  # one column is the gene/pathway name
    message(paste("Found ", num_clones, " different clones in the provided tibble.", sep = ""))
    ...
    # then use apply on the pairs with a function that takes two genes/pathways, and the clone tibble
    # which invokes is_diff_branch_ent_pair(ent1, ent2, clone_tbl)
    # and returns whether or not the pair is in different branches/clones (TRUE or FALSE)
    all_pairs_branch_info <- apply(all_pairs, 1, function(x){
      suppressMessages(is_diff_branch_ent_pair(x[1], x[2], clone_tbl))})
    ...
    # message to user
    message(paste("Of ", num_pairs, " total gene/pathway pairs, ", num_pairs_diff_branch, " occur in ",
                  "different branches/clones of the tumor.", sep = ""))
...
#'
#' Given two mutated genes or pathways and the clone tibble as described in
#' \code{\link{get_rate_diff_branch_ent_pair}}, this function returns \code{TRUE} or \code{FALSE}
#' for whether the pair is mutated in different branches/clones.
#'
...
#' in the tumor, e.g. 'clone1', 'clone2', 'clone3'. This tibble can be generated e.g. from the \code{Cloe} output.
#' @author Ariane L. Moore
#' @return \code{TRUE} or \code{FALSE} for whether or not the pair is mutated in different clones/in different
...
is_diff_branch_ent_pair <- function(ent1, ent2, clone_tbl) {
  ...
  ent1_profile_tbl <- clone_tbl %>% dplyr::filter(altered_entity == ent1) %>% dplyr::select(-altered_entity)
  ent2_profile_tbl <- clone_tbl %>% dplyr::filter(altered_entity == ent2) %>% dplyr::select(-altered_entity)
  ...
  if(is.na(ent1_profile)[1] || is.na(ent2_profile)[1]){
    warning(paste("At least one of the two entities ", ent1, " and ", ent2, " is not in the provided tibble.",
                  sep = "" ))
    return(FALSE)
  } else {
    
    # the product of these two profiles will only have ones where both occur in the same clone
    # i.e. sum of this will count how many clones they share
    num_shared_clones <- sum(ent1_profile*ent2_profile)
    message(paste("The pair ", ent1, " and ", ent2, " share ", num_shared_clones, " clone(s).", sep = ""))
...
#' which tree of the collection of tree inferences was used. This tibble can be generated e.g. from the \code{Cloe} output.
#' @author Ariane L. Moore
#' @return A list with two vectors: The numbers of how often gene pairs were mutated across trees, and the numbers of
#' how often they were clonally exclusive. The order of these two vectors is matching, i.e. the ith entry in each
...
get_hist_clon_excl <- function(clone_tbl) {
  ...
  # get the histogram of how often pairs were clonally exclusive/all trees
  # get all genes/pathways, here referred to as entity (ent)
  all_ents <- unique(as.character(clone_tbl$altered_entity))
  num_ents <- length(all_ents)
  ...
  if("patient_id" %in% colnames(clone_tbl)){
    ...
    message(paste0("Found ", num_trees, " different tree instances in the provided gene-to-clone tibble, ",
                   "and ", num_ents, " different mutated genes/pathways in total."))
  }
  ...
  message("Will compute the histograms for all pairs of how often they occur across tree instances, and how often they are clonally exclusive...")
  ...
  # then use apply on the pairs with a function that takes two genes/pathways, and the clone tibble
  # which invokes get_hist_clon_excl_this_pat_this_pair(ent1, ent2, clone_tbl)
  # and returns in how many trees they were both mutated and how often clonally exclusive
  all_pairs_clon_excl_over_num_trees_info <- apply(all_pairs, 1, function(x){
    this_res_num_trees_num_clon<-suppressMessages(get_hist_clon_excl_this_pat_this_pair(x[1], x[2], clone_tbl))
  ...
  # it can be that there are also zeros in the num_trees_pair_mutated, because we compute all pairs
  # by taking all ents from all tree inferences. It can be that one gene is only in one tree, and the other only in
  # a different tree. Then for this pair, the num_trees_pair would be zero. We do not want to include
  # pairs who are never mutated together in a tree. Hence, we exclude those
  idx_pairs_zero_times <- which(num_trees_pair_mutated == 0)
  if(length(idx_pairs_zero_times) > 0){
    num_trees_pair_mutated <- num_trees_pair_mutated[-idx_pairs_zero_times]
    num_times_clonal_excl <- num_times_clonal_excl[-idx_pairs_zero_times]
  }
  if(length(num_trees_pair_mutated) == 0 || length(num_times_clonal_excl) == 0){
   message(paste0("It seems like there was no gene/pathway pair at all that occurs in the same gene-to-clone assignment.",
                  " Please make sure that you have at least two genes/pathways assigned to clones in at least one of the tree instances."))
   return(list(0, 0))
  } else {
  
    # message to user
    message(paste("The median value for how often a gene pair occurs across the trees is ", median(num_trees_pair_mutated), ", and ",
                median(num_times_clonal_excl) , " is the median value for the pairs being clonally exclusive.", sep = ""))
...
#'
#' @title Check for a pair how often it was mutated in the current patient across trees, and how often also clonally exclusive.
...
#' which tree of the collection of tree inferences was used. This tibble can be generated e.g. from the \code{Cloe} output.
#' @author Ariane L. Moore
#' @return A vector with the values of in how many trees the pair was mutated, and in how many of those it was clonally exclusive.
#' @import 
#' dplyr
#' @examples
#' clone_tbl <- dplyr::as.tbl( as.data.frame( cbind(
...
get_hist_clon_excl_this_pat_this_pair <- function(entA, entB, clone_tbl){
  ...
  stopifnot("clone1" %in% colnames(clone_tbl))
  stopifnot("tree_id" %in% colnames(clone_tbl))
  if("patient_id" %in% colnames(clone_tbl) || "file_name" %in% colnames(clone_tbl)){
    stop("The clone tibble is expected to have only the altered_entity, tree_id, and clones columns, but not file_name, or patient_id!")
  }
  
  for(this_col in colnames(clone_tbl)){ # sanity check
    if(this_col != "altered_entity" && !grepl("clone", this_col) && this_col != "tree_id"){
      # if there still is a column in this_clone_tbl that is not either a clone or an altered entity, or the tree_id
      # this will be problematic, because it will be counted as a clone and hence may 
      # lead to erroneous numbers of clonal exclusivity
      warning(paste0("The column ", this_col, " in the clone tibble may be accidentally counted as a clone."))
    }
  }
  ...
  message(paste("Found ", num_trees, " different tree ids in the clone tbl.", sep = ""))
  
  # get the info of num_trees and clon excl
  this_pair_across_trees <- vapply(all_tree_ids, function(x){
    this_tree_clone_tbl <- clone_tbl %>% dplyr::filter(tree_id == x) %>% dplyr::select(-tree_id)
    all_ents_this_tree <- as.character(this_tree_clone_tbl$altered_entity)
    if(entA %in% all_ents_this_tree && entB %in% all_ents_this_tree){
      this_pair_occurs <- 1
      this_pair_clon_excl <- sum(suppressMessages(is_diff_branch_ent_pair(entA, entB, this_tree_clone_tbl)))
    } else{
...
#'
#' Given a clone tibble as created with \code{\link{create_tbl_ent_clones}} from one patient 
#' and where the entities were possibly mapped from genes to pathways, this function checks whether there
#' were several entities mapped to the same new entity. If so, the clone profile will be merged.
#' This can be the case, for instance, of two mutated genes are in the same pathway(s).
#'
#' @title Merge identical entities in clone tibble from one patient
#' @param clone_tbl The clone tibble as generated with \code{\link{create_tbl_ent_clones}} from one patient.
#' @author Ariane L. Moore
#' @return The same tibble but in case there were several identical genes/pathways in the same patient with
#' different clone profiles, their profile will be merged together. This can happen if, e.g. two genes with
#' different clone profiles are in the same pathway. When mapping them to the pathways, there will be
#' two identical 'altered_entities' with different clone profiles. These profiles would be merged by this
#' function because the pathway is affected in the union of clones were the two genes were mutated.
#' @import 
#' dplyr
#' tibble
#' @examples
...
merge_clones_identical_ents <- function(clone_tbl) {
  ...
  if("tree_id" %in% colnames(clone_tbl)){
    stop("The clone tibble is expected to have only the altered_entity, patient_id, file_name, and clones columns, but not tree_id!")
  }
  ...
  # message to user
  message(paste("The clone tibble from patient ", this_pat," contains ", num_entries, " entries.", sep = ""))
  message(paste("There are ", num_clones, " different clones specified.", sep = ""))
  ...
  message(paste("There are ", num_unique_ents, " different mutated genes/pathways in the tibble.", sep = ""))
  
  # and if so, merge them together
  merged_ents_clone_tbl_list <- lapply(all_sorted_unique_ents, function(x){
    # extract the tibble with just the clone profiles for the current entity
    ...
    merged_clone_profile <- ifelse(apply(this_ent_tbl, 2, sum) > 0, 1, 0)
    ...
    for(i in 1:num_clones){
      ...
      names(this_ent_merged_tbl)[names(this_ent_merged_tbl) == "new_col"] <- clone_name
    }
  ...
  final_merged_clone_tbl <- do.call("rbind", merged_ents_clone_tbl_list)
  ...
  message(paste("Now there are ", num_entries, " different mutated genes/pathways from patient ",
                this_pat, ".", sep = ""))
...
#'
#' Given a clone tibble as created with \code{\link{create_tbl_ent_clones}} this function extracts the information,
#' how many clones there are in each patient. The counted clones will be those with at least one non-zero entry, i.e.
#' at least one gene/pathway assigned to the clone.
#'
#' @title Extract number of clones.
#' @param clone_tbl The tibble generated with \code{\link{create_tbl_ent_clones}}.
#' @author Ariane L. Moore
#' @return A named vector with the number of clones in each patient. The name of each element is the respective
#' patient_id.
#' @import 
#' dplyr
#' @examples
...
extract_num_clones_tbl <- function(clone_tbl) {
  ...
  stopifnot("clone1" %in% colnames(clone_tbl))
  stopifnot("clone2" %in% colnames(clone_tbl))
  stopifnot(!"tree_id" %in% colnames(clone_tbl)) # this should not be in here because otherwise it will be counted as a clone

  # message to user
  num_entries <- dim(clone_tbl)[1]
  message(paste("", num_entries, " alterations in total found in the provided tibble.", sep = ""))
  num_clones_upper_bound <- dim(clone_tbl)[2]-3
  message(paste("", num_clones_upper_bound, " is assumed to be the upper bound of number of clones.", sep = ""))

  # extract all the patient id's
  all_pats_tbl <- clone_tbl %>% dplyr::select(patient_id) %>% dplyr::distinct()
  all_pats <- as.vector(as.data.frame(all_pats_tbl)[,1])

  num_clones <- sapply(all_pats, function(x){
  ...
  message(paste("There are ", min_num_clones, "-", max_num_clones, " clones in each patient.", sep = ""))
  return(num_clones)
}
...
#' @param clone_tbl_all_trees The tibble containing the information of which gene/pathway is mutated in which
...
#' @param pairs_of_interest_tbl A tibble containing pairs of mutated genes/pathways. More precisely, it contains the columns
#' 'entity_A' and 'entity_B'.
#' @author Ariane L. Moore
#' @return A tibble similar to the input \code{pairs_of_interest_tbl} but with two additional columns, namely 'mutated_in', and
...
take_pairs_and_get_patients <- function(clone_tbl_all_trees, pairs_of_interest_tbl){
  ...
  num_pats <- length(unique(as.character(clone_tbl_all_trees$patient_id)))
  num_trees <- length(all_tree_ids)
  num_pairs <- dim(pairs_of_interest_tbl)[1]
  message(paste("Found ", num_pairs, " pairs of interest for which the patients will be extracted.", sep = ""))
  message(paste("There are ", num_pats, " different patients in the clone tibble and ",
                "the number of tree inferences is ", num_trees, ".", sep = ""))
  num_min_trees_clon_excl <- floor((num_trees/2)+1)
  message(paste0("A pair is counted as clonally exclusive in a patient, if it clonally exclusive in the majority",
                 " of tree inferences, i.e. in at least ", num_min_trees_clon_excl, " trees of a patient."))
  
  # find out for each pair separately in which patients it is mutated and clonally exclusive
  pairs_res_pats <- apply(pairs_of_interest_tbl, 1, function(x){
    ...
    pat_ids_all_trees <- sapply(all_tree_ids, function(y){
      this_tree_tbl <- this_pair_pats_and_trees %>% dplyr::filter(tree_id == y)
      pat_ids_in_which_both_mutated <- as.character(as.vector(as.data.frame(this_tree_tbl %>%
    ...
    # now we extract from the clone tbl only the patients in which they are both mutated
    this_pair_pats_mut_clon_tbl <- clone_tbl_all_trees %>%
      dplyr::filter(altered_entity %in% x) %>%
      dplyr::filter(patient_id %in% these_pats_mut)

    # for each pat in which they are mutated, get number of times it was clonally exclusive
    pair_num_clon_excl <- vapply(these_pats_mut, function(y){
      this_clone_tbl <- this_pair_pats_mut_clon_tbl %>%     # take the clone tibble
        dplyr::filter(patient_id == y) %>%                      # just for the current patient
        dplyr::select(-file_name, -patient_id)
      for(this_col in colnames(this_clone_tbl)){ # sanity check
        if(this_col != "altered_entity" && !grepl("clone", this_col) && this_col != "tree_id"){
          # if there still is a column in this_clone_tbl that is not either a clone or an altered entity, or the tree_id
          # this will be problematic, because it will be counted as a clone and hence may 
          # lead to erroneous numbers of clonal exclusivity
      ...
      num_trees_num_clon_excl_this_pat <- suppressMessages(get_hist_clon_excl_this_pat_this_pair(entA, entB, this_clone_tbl))
      return(num_trees_num_clon_excl_this_pat[2])
    }, numeric(1))
    stopifnot(length(pair_num_clon_excl) == length(these_pats_mut))

    
    # now we construct the stings to return, that is the string with all semicolon separated pat ids in which the pair is mutated
    # and the string with all semicolon separated pat ids in which they are clonally exlcusive in the majority of trees
    this_pair_mutated_in<-paste(these_pats_mut, sep = "", collapse = ";")
    this_pair_clon_excl_in_vec <- names(pair_num_clon_excl[which(pair_num_clon_excl >= num_min_trees_clon_excl)])
    this_pair_clon_excl_in <- paste(this_pair_clon_excl_in_vec, sep = "", collapse = ";")
  ...
  res_tbl <- tibble::as_tibble(cbind(as.data.frame(pairs_of_interest_tbl), cbind(mutated_in, clonally_exclusive_in)))
...
#'
#' After having extracted the pairs of interest, it is of interest to know the genes hgnc symbols of the pairs. Here, it is
#' assumed that the current gene identifier of the pairs are ensembl gene ids. They will be mapped to the corresponding
#' hgnc symbols.
#'
#' @title Map the ensembl gene ids to hgnc symbols from a tibble
#' @param pairs_of_interest_tbl A tibble containing pairs of mutated genes/pathways. More precisely, it contains the columns
#' 'entity_A' and 'entity_B'.
#' @param all_genes_tbl A tibble with all genes ensembl id's and hgnc symbols. Can be created with \code{\link{create_ensembl_gene_tbl_hg}}.
#' @author Ariane L. Moore
#' @return A tibble similar to the input \code{pairs_of_interest_tbl} but with two additional columns, namely 'hgnc_gene_A', and
#' 'hgnc_gene_B'. The column 'hgnc_gene_A' contains the hgnc gene symbol of 'entity_A', and the column 'hgnc_gene_B' the one of 'entity_B'.
#' @import 
#' dplyr
#' tibble
#' @examples
...
map_pairs_to_hgnc_symbols <- function(pairs_of_interest_tbl, all_genes_tbl){
  ...
  these_ens_ids_A <- as.vector(as.data.frame(pairs_of_interest_tbl %>% dplyr::select(entity_A))[,1])
  hgnc_gene_A <- c()
  ...
  these_ens_ids_B <- as.vector(as.data.frame(pairs_of_interest_tbl %>% dplyr::select(entity_B))[,1])
  hgnc_gene_B <- c()
  ...
  pairs_of_interest_tbl_hgnc <- tibble::as_tibble(cbind(as.data.frame(pairs_of_interest_tbl), cbind(hgnc_gene_A, hgnc_gene_B)))
...
#'
#' After having extracted the significant pairs. The tibble can be saved as a tab-separated file. It is assumed that the
#' input tibble has the columns 'hgnc_gene_A', 'hgnc_gene_B', 'pval', 'qval', 'mutated_in', 'clonally_exclusive_in'.
#'
#' @title Write resulting significant pairs to disk
#' @param sig_pairs The tibble containing the significant pairs of mutated genes/pathways.
#' @param avg_rates_m The average rates of clonal exclusivity for each patient. The name of each rate is the respective 
#' patient identifier.
#' @param tsv_file The path to the tsv-file to which the results should be written.
#' @param num_digits The number of digits after the comma of the average rate m, the p-value and the q-value (adjusted p-value). 
#' Default: 2.
#' @author Ariane L. Moore
#' @return The tibble that is written to disk. It has the columns 'Gene A', 'Gene B', 'P-value', 'Adjusted p-value', 
...
write_res_pairs_to_disk <- function(sig_pairs, avg_rates_m, tsv_file, num_digits = 2){
  ...
  # instead of just the patient id (e.g. "pat1; pat2") we want also the string Patient and the rate in parenthesis (e.g. "Patient pat1 (0.0034); Patient pat2 (0.0021)")
  mutated_in_with_patient_and_rates <- vapply(sig_pairs$mutated_in,
                                 function(x){
                                   these_pats_label <- unlist(strsplit(x, ";"))
                                   
                                   these_pats_label_with_rates <- c()
                                   for(this_pat in these_pats_label){
                                     this_pat <- trimws(this_pat, which = "both")
                                     # make sure that the patients labels are also among the named rates
                                     stopifnot(this_pat %in% names(avg_rates_m))
                                     
                                     this_new_label <-paste0("Patient ", this_pat,
                                                             " (", round(avg_rates_m[which(names(avg_rates_m) == this_pat)], digits = num_digits), ")")
                                     these_pats_label_with_rates <- c(these_pats_label_with_rates, this_new_label)
                                   }
                                   return(paste(sort(these_pats_label_with_rates), collapse = "; "))
                                 },
                                 character(1))
  # instead of just the patient id (e.g. "pat1; pat2") we want also the string Patient (e.g. "Patient pat1; Patient pat2")
  clonally_excl_in_with_patient <- vapply(sig_pairs$clonally_exclusive_in,
                                          ...
                                          character(1))
  # remove the names, because otherwise the results tibble has weird rownames
  ...
  table_to_save <- data.frame(GeneA = sig_pairs$hgnc_gene_A,
                              ...
                              Pvalue = round(as.numeric(as.vector(sig_pairs$pval)), digits = num_digits),
                              AdjustedPvalue = round(as.numeric(as.vector(sig_pairs$qval)), digits = num_digits),
  ...
  colnames(table_to_save) <- c("Gene A", "Gene B", "P-value", "Adjusted p-value",
                                       "Mutated in (rate)", "Clonally exclusive in")
  # write it to disk
...
#' 
#' After having created the tibble with all gene-to-clone assignments from all patients and the whole collection
#' of trees, we're interested in how many patients tha pairs are mutated in. This function creats a histogram
...
#' @author Ariane L. Moore
#' @return The tibble that summarizes the number of pairs that occur in how many patients.
#' @import 
#' dplyr
#' utils
#' @examples
...
pairs_in_patients_hist <- function(clone_tbl){
  ...
  message(paste("Found ", num_pats, " different patient id's in the provided tibble.", sep = ""))
  ...
  message(paste0("There are in total ", num_pairs, " pairs of genes/pathways mutated in at least one patient."))
  ...
  pairs_num_pats_tally <- tibble::tibble(pairs = all_pairs_all_pats_one_string) %>% 
    dplyr::group_by(pairs) %>% 
    dplyr::tally() %>%
    dplyr::mutate(patient_count = n) %>%
    dplyr::select(pairs, patient_count)
  
  num_pairs_num_pats_tally <- pairs_num_pats_tally %>% dplyr::select(patient_count) %>%
  ...
  message(paste0("There are in total ", num_pairs_atLeast2, " pairs of genes/pathways mutated in at least 2 patients."))
  
  # check what is the maximum n that pairs are mutated in how many patients
  ...
  message(paste0("The maximum number of patients that a pair is mutated in is ", max_num_pat_pairs,
                   ". Please run the function 'generate_ecdf_test_stat' with ",
