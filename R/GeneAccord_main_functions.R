...
#'
#' @title Detection of clonally exclusive gene or pathway pairs in 
...
#' @param alternative The character indicating whether pairs should only 
#' be tested if delta > 0 or if all pairs should be tested.
#' Can be one of "greater" or "two.sided". Default: "greater".
#' @param genes_of_interest A character vector of genes to test for 
...
#' @param AND_OR If \code{genes_of_interest} is specified, this indicator 
...
#' @return A tibble containing the test result for each pair of mutated 
...
GeneAccord <- function(clone_tbl, avg_rates_m, ecdf_list, 
    ...
    ## already try to filter such that only those pairs remain 
    ## which are mutated in two or more
    min_num_pat <- 2
    ...
    if(length(ecdf_list) < max_num_pat_pairs){
        ...
        "If you have more time, please re-run the function ",
        "'generate_ecdf_test_stat' with ",
    ...
    if(spec_genes){ # if TRUE, only test genes of interest
        ## sanity check: make sure that the genes of interest are among 
        ...
        if(AND_OR == "AND"){
            ...
            pairs_to_test <- apply(all_pairs, 1, function(x){
                ...
                if(this_ent_A %in% genes_of_interest && 
                    this_ent_B %in% genes_of_interest){
                    return(x)
                }
            })
        } else if(AND_OR == "OR"){
    ...
    if(is.null(pairs_to_test)){
        ...
        if(spec_genes){ # if only a subset of all pairs is being tested
            pairs_to_test <- 
                matrix(unlist(pairs_to_test[unlist(lapply(pairs_to_test, 
        ...
        ## num_patients=num_shared_pats, mle_delta=mle_delta,
        ## test_statistic=test_stat) for each pair
        all_tests <- list()
      
        all_tests <- apply(pairs_to_test, 1, function(x){
            suppressMessages(ecdf_lr_test_clon_excl_avg_rate(x[1], x[2], 
                clone_tbl, 
                avg_rates_m, 
                ecdf_list, 
                alternative))})
        ...
        res_all_pairs_minNumPat2$mle_delta <- 
